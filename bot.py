...
CHOOSE_CLIENT, CHOOSE_DATE, CHOOSE_FORMAT = range(3)
...
async def ask_date_range(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text("Date range select করুন:", reply_markup=reply_markup)
    return CHOOSE_DATE
...
async def date_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "date_today":
        context.user_data["date_start"] = str(today)
        context.user_data["date_stop"] = str(today)
        context.user_data["date_label"] = "Today"
    elif data == "date_yesterday":
        yesterday = today - timedelta(days=1)
        context.user_data["date_start"] = str(yesterday)
        context.user_data["date_stop"] = str(yesterday)
        context.user_data["date_label"] = "Yesterday"
    elif data == "date_7d":
        context.user_data["date_start"] = str(today - timedelta(days=7))
        context.user_data["date_stop"] = str(today)
        context.user_data["date_label"] = "Last 7 days"
    elif data == "date_15d":
        context.user_data["date_start"] = str(today - timedelta(days=15))
        context.user_data["date_stop"] = str(today)
        context.user_data["date_label"] = "Last 15 days"
    elif data == "date_30d":
        context.user_data["date_start"] = str(today - timedelta(days=30))
        context.user_data["date_stop"] = str(today)
        context.user_data["date_label"] = "Last 30 days"
    elif data == "date_90d":
        context.user_data["date_start"] = str(today - timedelta(days=90))
        context.user_data["date_stop"] = str(today)
        context.user_data["date_label"] = "Last 90 days"
    elif data == "date_custom":
        await query.edit_message_text(
            "Custom date range দিন (format: YYYY-MM-DD to YYYY-MM-DD)\n"
...
async def ask_format(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text("Report format select করুন:", reply_markup=reply_markup)
...
async def generate_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for client_name in selected_clients:
        ...
        try:
            ...
            if fmt in ("text", "both"):
                text_report = report_gen.build_text_report(
                    client_name=client_name,
                    date_label=date_label,
                    data=data,
                )
                await bot.send_message(chat_id=chat_id, text=text_report, parse_mode="HTML")
            if fmt in ("pdf", "both"):
                pdf_path = report_gen.build_pdf_report(
                    client_name=client_name,
                    date_label=date_label,
                    data=data,
                )
            ...
            await bot.send_message(
                chat_id=chat_id,
                text=f"{client_name} এর report generate করতে সমস্যা হয়েছে।\nError: {str(e)}"
            )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "/clients — client list\n"
        "/cancel — বাতিল করুন\n\n"
        "Date options: Today, Yesterday, 7/15/30/90 days, Custom\n"
        "Format options: Text, PDF, Text+PDF"
    )
    await update.message.reply_text(text)


# Webhook handler for Render Web Service (free tier)
...
async def health_check(request):
    return web.Response(text="SKF Ads Reporter Bot is running!")
...
def main():
    ...
    if webhook_url:
        # Render Web Service — webhook mode (free tier)
